...
class Core:
                ...
                try:
                    ...
                    if not self.current_model or self.current_model.__class__.__name__.lower() != request["model"]:
                        ...
                        if request["model"] == "gemini":
                            self.current_model = GeminiModel()  # No settings parameter needed
    ...
    def execute(self, text):
        ...
        try:
            ...
            screenshot = self.screen.get_screenshot_file()
            ...
            while not self.stop_requested:
                # Get next instruction from model
                result = self.current_model.get_instructions_for_objective(text, step_num)
                ...
                if result.get('done', False):
                    self.send_status(f"✅ {result.get('done', 'Task completed')}")
                    break
                    
                commands = result.get('commands', [])
                if not commands:
                    commands = result.get('steps', [])  # Try alternative key
